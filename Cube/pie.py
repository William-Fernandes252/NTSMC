...
mean = np.array([rows[1][0], rows[1][1], rows[1][2], rows[1][3], 
                 rows[1][4]]).astype(np.float64)
std = np.array([rows[2][0], rows[2][1], rows[2][2], rows[2][3], 
                rows[2][4]]).astype(np.float64)

# Create the graph:
wedges, texts, autotexts = graph.pie(mean, 
                                     autopct = lambda pct: 
                                     computePercentage(pct, mean, std), 
                                     textprops = dict(color = "k"), 
                                     radius = 1.1)

# Defining the subtittles box:
graph.legend(wedges, quantities, title = "Eventos", loc = "center left", 
