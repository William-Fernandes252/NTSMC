...
/* Computes the distance between the given point and the origin. */
#define DISTANCE(x, y, z) (sqrt(SQUARE(x) + SQUARE(y) + SQUARE(z))) 
...
typedef union {
    fuel u_diox;       /* Fuel simulated in the experiment (uranium diox.).  */
...
/*
    init_fission_neutrons:  Given the coordinates of the incident particle, 
                            compute the coordites of the particles emitted 
                            by the fission event and stores then in the memory 
...
/*
    select_path:        Selects the walk of the particle to the site where 
                        its next interaction with the medium will happen.
*/
void 
...
/*
    select_interaction:     Given the location of the particle in the medium, 
                            selects a interaction for it with some nuclide in 
                            the surroundings. Returns the number of additional 
...
/*
    iso_elastic_scattering:     Simulates the scattering of particles, that 
                                ...
                                direction after a non-absorption interaction 
                                is isotropic. For simplicity, the cosine of the 
...
/*
    punctual_source:    Simulates the emission of particles by a punctual 
                        source located in a selected point of the sistem. 
*/
void 
punctual_source(neutron *const particle, const unsigned long *const h);
