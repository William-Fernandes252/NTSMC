...
print_report(const unsigned long n, const unsigned long r, 
	...
	for(int l = 0; l != LAYERS; l++)
	    printf("%-11d%13.5e %c %-13.5e\n", (l+1), results->coll_mean[l], 
			   (char)241, results->coll_dev[l]);

	puts("\n\nAverage number of escapes per layer:\n");
	printf("%-12s%-s\n", "Layer", "Escapes");	
	for(int l = 0; l != LAYERS; l++)
	    printf("%-11d%13.5e %c %-13.5e\n", (l+1), results->esc_mean[l], 
		        (char)241, results->esc_dev[l]);

	puts("\n\nAverage total flux vs. radius:\n");
    printf("%-12s%-s\n", "Radius", "Total flux");
	for(int l = 0; l != LAYERS; l++)
	    printf("%-11.2f%13.5e %c %-13.5e\n", ((double)(l+1)*(RADIUS/LAYERS)), 
				results->flux_mean[l], (char)241, results->flux_dev[l]);
...
write_report(const unsigned long n, const unsigned long r, 
	...
	for(int l = 0; l != LAYERS; l++)
	    fprintf(report, "%-8d%13.5e +/- %-13.5e\n", (l+1), 
				results->coll_mean[l],results->coll_dev[l]);

	fputs("\nAverage number of collisions per layer:\n", report);
	fprintf(report, "%-9s%-s\n", "Layer", "Escapes");	
	for(int l = 0; l != LAYERS; l++)
	    fprintf(report, "%-8d%13.5e +/- %-13.5e\n", (l+1), 
				results->esc_mean[l],results->esc_dev[l]);

	fputs("\nAverage total flux vs. radius:\n", report);
    fprintf(report, "%-9s%-s\n", "Radius", "Total flux");
	for(int l = 0; l != LAYERS; l++)
	    fprintf(report, "%-8.2f%13.5e +/- %-13.5e\n", 
				((double)(l+1)*(RADIUS/LAYERS)), results->flux_mean[l], 
				results->flux_dev[l]);
		...
		case 'c':
			...
			for(int l = 0; l != LAYERS; l++)
    			fprintf(report, "%d%c%.5e%c%.5e\n", (l+1), sep, 
						results->coll_mean[l], sep, results->coll_dev[l]);
			break;
		...
		case 'f':
			...
			for(int l = 0; l != LAYERS; l++)
				fprintf(report, "%.5f%c%.5e%c%.5e\n", 
						((double)(l+1)*(RADIUS/LAYERS)), sep, 
						results->flux_mean[l], sep, results->flux_dev[l]);
			break;
		case 'e':
			if(new_report) 
			...
			for(int l = 0; l != LAYERS; l++)
    			fprintf(report, "%d%c%.5e%c%.5e\n", (l+1), 
						sep, results->esc_mean[l], sep, results->esc_dev[l]);
			break;
