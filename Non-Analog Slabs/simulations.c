...
void init_fission_neutrons(neutron *const incident_n, neutron *const fission_n, const medium *const slabs) {
    ...
    fission_n->weight = incident_n->weight * slabs->comp.u_diox.n_fission;

    // Reduce the weight of the incident particle:
    incident_n->weight = (slabs->comp.u_diox.fission_cs - slabs->comp.u_diox.fission_cs)/slabs->comp.u_diox.fission_cs;
...
bool select_interaction(neutron *particle, const medium *const slabs) {
    ...
    switch(slabs->tag) {
        case FUEL:
            ...
            wr_factor = (t - slabs->comp.u_diox.abs_cs)/t;
            break;
        case MODERATOR:
            ...
            wr_factor = (t - slabs->comp.water.abs_cs)/t;
            break;
    }
...
void punctual_source(neutron *particle, const unsigned long *const h) {
    ...
    // Initial position, energy and weight:
    particle[*h].x = 0.0;
    particle[*h].y = 0.0;
    particle[*h].z = 0.0;
