...
void write_report(const unsigned long n, const unsigned long r, const result *const results) {
	...
	fprintf(report, "Relative abs.:\t%13.5e%17.5e\n\n", results->mod_frac_m, results->mod_frac_d);
	fprintf(report, "Relative ref.:\t%13.5e%17.5e\n\n", results->ref_frac_m, results->ref_frac_d);
	fprintf(report, "Relative esc.:\t%13.5e%17.5e\n\n", results->esc_frac_m, results->esc_frac_d);
...
void csv_report(const unsigned long n, const unsigned long r, const result *const results) {
	...
	if(option == 's') {
		...
		fprintf(report, "Histories%cRepetitions%cMod. mean%cMod. std.%cRef. mean%cRef. std.%cEsc. mean%cEsc. std.%cRelative mod. mean%cRelative mod. std.%cRelative ref. mean%cRelative ref. std.%cRelative esc. mean%cRelative esc. std.\n", sep, sep, sep, sep, sep, sep, sep, sep, sep, sep, sep, sep, sep);
		fprintf(report, "%lu%c%lu%c%.5e%c%.5e%c%.5e%c%.5e%c%.5e%c%.5e%c%.5e%c%.5e%c%.5e%c%.5e%c%.5e%c%.5e\n", n, sep, r, sep, results->mod_m, sep, results->mod_d, sep, results->ref_m, sep, results->ref_d, sep, results->esc_m, sep, results->esc_d, sep, results->mod_frac_m, sep, results->mod_frac_d, sep, results->ref_frac_m, sep, results->ref_frac_d, sep, results->esc_frac_m, sep, results->esc_frac_d);
