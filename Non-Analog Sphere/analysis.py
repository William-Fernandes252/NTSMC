...
def create_graph(option, x, x_label, y, y_label, err, color):
    ...
    if option == 'n':
        graph.errorbar(x, y, yerr = err, color = color, ecolor = 'black', capsize = 2)
    elif option == 's':
        graph.bar(x, y, yerr = err, align = 'center', color = color, ecolor = 'black', capsize = 1, log = True)
    ...
    # Tittle of the graph:
    graph.set_title("Estimatives of the " + y_label.lower() + " per " + x_label.lower())
