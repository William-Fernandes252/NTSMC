...
void sphere(const unsigned long his, const unsigned long rep, result *const results) {
    ...
    double  n_collisions[LAYERS],   /* Stores the number of collisions that occurred in each layer of the sphere.                               */
            n_escapes[LAYERS],      /* Stores the number of particles that left the system from each region.                                    */
            d;                      /* Keep track of the distance between a particle and the source.                                            */
    unsigned int curr_layer;        /* Keep track of the layer of the sphere where the particle is located.                                     */
    unsigned int last_layer;        /* Keep track of the layer of the sphere where the particle was located before its next random random walk. */
    ...
    for(unsigned long r = 0; r < rep; r++) {
        ...
        for(unsigned long h = 0; h < n; h++) {
            ...
            while(true) {
                ...
                if(d > RADIUS) {
                    n_escapes[last_layer] += particle[h].weight;
                    break;
                }

                init_cross_sections(&particle[h], &sphere, d);

                if(select_interaction(&particle[h], &sphere)) {
                    n += 1;
                    particle = realloc(particle, sizeof(neutron) * n);
                    // Store the coordinates of the fission particle to follow later:
                    init_fission_neutrons(&particle[h], &particle[n-1], &sphere);
                }
  
                n_collisions[curr_layer] += particle[h].weight;
    ...
    if(rep > 1) {
        for(unsigned int l = 0; l < LAYERS; l++) {
            results->coll_dev[l] = sqrt((results->coll_dev[l]/rep) - SQUARE(results->coll_mean[l]))/(rep - 1);
            results->esc_dev[l] = sqrt((results->esc_dev[l]/rep) - SQUARE(results->esc_mean[l]))/(rep - 1);
