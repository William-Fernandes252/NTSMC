...
void init_fission_neutrons(neutron *const incident_n, neutron *const fission_n, const medium *const sphere) {
    ...
    fission_n->weight = incident_n->weight * sphere->comp.u_diox.n_fission;

    // Reduce the weight of the incident particle:
    incident_n->weight = (sphere->comp.u_diox.fission_cs - sphere->comp.u_diox.fission_cs)/sphere->comp.u_diox.fission_cs;
...
bool select_interaction(neutron *const particle, const medium *const sphere) {
    ...
    switch(sphere->tag) {
        case FUEL:
            ...
            wr_factor = (t - sphere->comp.u_diox.abs_cs)/t;
            break;
        case MODERATOR:
            ...
            fission_p = 0.00000;
            mass_num = sphere->comp.u_diox.mass_num;
            wr_factor = (t - sphere->comp.water.abs_cs)/t;
            break;
    }
