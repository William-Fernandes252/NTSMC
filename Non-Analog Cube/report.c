...
void write_csv(FILE *const report, char option, char sep, bool new_report, const result *const results) {
	...
	switch(option) {
		...
		case 'a':
			...
			fprintf(report, "%.5e%c%.5e%c%.5e%c%.5e%c%.5e%c%.5e%c%.5e%c%.5e\n", results->mod_m, sep, results->ref_m, sep, results->det_m, sep, results->esc_m, sep, results->hist_m, sep, results->stp_m, sep, results->frp_m);
			fprintf(report, "%.5e%c%.5e%c%.5e%c%.5e%c%.5e%c%.5e%c%.5e%c%.5e\n", results->mod_d, sep, results->ref_d, sep, results->det_d, sep, results->esc_d, sep, results->hist_d, sep, results->stp_d, sep, results->frp_d);
			break;

		case 'r':
			if(new_report)
				fprintf(report,"Moderations%cAbsorptions%cReflections%cDetections%cEscapes", sep, sep, sep, sep, sep, sep, sep, sep, sep, sep, sep, sep, sep);
			fprintf(report, "%.5e%c%.5e%c%.5e%c%.5e%c%.5e%c%.5e%c%.5e\n", results->mod_m, sep, results->ref_m, sep, results->det_m, sep, results->esc_m, sep, results->stp_m, sep, results->frp_m);
			fprintf(report, "%.5e%c%.5e%c%.5e%c%.5e%c%.5e\n", results->mod_d, sep, results->ref_d, sep, results->det_d, sep, results->esc_d);
			break;
