...
void cube(const unsigned long his, const unsigned long rep, result *const results) {
    ...
    for(unsigned long r = 0; r < rep; r++) {
        ...
        for(unsigned long h = 0; h < n; h++) {
            ...
            while((FUEL_X1 <= particle[h].x && particle[h].x <= MODERATOR_X2) && (Y1 <= particle[h].y && particle[h].y <= Y2) && (Z1 <= particle[h].z && particle[h].z <= Z2)) {

                distance += select_path(&particle[h], &cube) * particle[h].weight;
                steps += particle[h].weight;

                if(particle[h].x < FUEL_X1) {
                    reflections += particle[h].weight;
                    break;
                }
                else if((particle[h].x < FUEL_X1 || MODERATOR_X2 < particle[h].x) || (particle[h].y < Y1 || Y2 < particle[h].y) || (particle[h].z < Z1 || Z2 < particle[h].z)) {
                    escapes += particle[h].weight;
                    break;
                }
                else if((DETECTOR_X1 <= particle[h].x && particle[h].x <= DETECTOR_X2) && (DETECTOR_Y1 <= particle[h].y && particle[h].y <= DETECTOR_Y2) && (DETECTOR_Z1 <= particle[h].z && particle[h].z <= DETECTOR_Z2)) {
                    if(!particle[h].detection) {
                        particle[h].detection = true;
                        detections += particle[h].weight;
                    }
                }

                init_cross_sections(&particle[h], &cube);

                if(select_interaction(&particle[h], &cube)) {
                    n += 1;
                    particle = realloc(particle, sizeof(neutron) * n);
                    // Store the coordinates of the fission particle to follow later:
                    init_fission_neutrons(&particle[h], &particle[n-1], &cube);
                }
    ...
    if(rep > 1) {
        ...
        results->det_d = sqrt((results->det_d/rep) - SQUARE(results->det_m))/(rep - 1);
        ...
        results->det_frac_d = sqrt((results->det_frac_d/rep) - SQUARE(results->det_frac_m))/(rep - 1);
        ...
        results->stp_d = sqrt((results->stp_d/rep) - SQUARE(results->stp_m))/(rep - 1);
        results->frp_d = sqrt((results->frp_d/rep) - SQUARE(results->frp_m))/(rep - 1);
    }
